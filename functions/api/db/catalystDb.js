...
// ---------- Row API helpers --------------------------------------------------
// New rows get a random ~10-digit id instead of the small sequential 1, 2, 3
// the identity sequence would hand out. The PK columns are `bigint generated BY
// DEFAULT as identity`, so an explicit id is allowed. Kept under 2^53 so JS
// represents it exactly; returned as a string to match the ROWID shape.
function genId() {
...
async function insert(req, table, row) {
  ...
  // Retry only guards against the astronomically unlikely PK collision.
  for (let attempt = 0; attempt < 6; attempt++) {
    const keys = [...(hasId ? ['id'] : []), ...entries.map(([k]) => k)];
    const vals = [...(hasId ? [genId()] : []), ...entries.map(([, v]) => coerce(v))];
    if (!keys.length) {
    ...
    try {
      ...
      if (hasId && e && e.code === '23505' && attempt < 5) continue; // PK clash → new id
      throw e;
    }
  }
}
...
async function bulkInsert(req, table, rows) {
  ...
  const allKeys = [...(hasId ? ['id'] : []), ...keys];
  if (!allKeys.length) return [];
  for (let attempt = 0; attempt < 6; attempt++) {
    const params = [];
    const tuples = rows.map((r) => {
      const ph = [];
      if (hasId) { params.push(genId()); ph.push(`$${params.length}`); }
      keys.forEach((k) => { params.push(coerce(r[k])); ph.push(`$${params.length}`); });
      return `(${ph.join(', ')})`;
    });
    const sql = `insert into ${qi(t)} (${allKeys.map(qi).join(', ')}) values ${tuples.join(', ')} returning *`;
    try {
      const { rows: out } = await query(sql, params);
      return out.map(catalystify);
    } catch (e) {
      if (hasId && e && e.code === '23505' && attempt < 5) continue;
      throw e;
    }
  }
}
